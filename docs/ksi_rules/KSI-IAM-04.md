...
PASS Excellent modern authorization model (100%): PASS IAM Identity Center configured: 1 active instance(s)
- PASS Permission sets deployed: 3 sets (modern role-based access)
- PASS Modern authorization model: Permission sets provide role-based, attribute-based, and just-in-time access
- PASS Active permission sets: AdministratorAccess, Production, ReadOnlyAccess
- PASS Role-based access pattern: 32 user-oriented roles vs 2 users (23 service roles)
- PASS Controlled user count: 2 users (excellent for least privilege)
- PASS Controlled policy count: 2 custom policies (good least privilege)
- PASS SSO session inferred: Identity Center active with permission sets (ARN redacted for security)
