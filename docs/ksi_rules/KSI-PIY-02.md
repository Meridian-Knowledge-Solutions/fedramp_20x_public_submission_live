# KSI-PIY-02: Have policies outlining the security objectives of all information resources

## Overview

**Category:** Policy and Inventory
**Status:** PASS
**Last Check:** 2025-07-09 02:40

**What it validates:** Have policies outlining the security objectives of all information resources

**Why it matters:** Manual evidence required - security policies and objectives documentation

## Validation Method

1. **Manual Review:** Check evidence_v2/KSI-PIY-02/ for security_policy.pdf, information_security_objectives.pdf, and resource_security_standards.pdf

## Latest Results

- PASS Security policies outlining objectives: PASS Core security policies: personnel_security_policy.pdf, data_classification_handling_policy.pdf, security_policy_collection_repository.zip

---
*Generated 2025-07-09 02:40 UTC*