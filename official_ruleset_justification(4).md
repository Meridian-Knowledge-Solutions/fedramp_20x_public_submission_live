...
```python
# Actual error handling patterns from CLI assertion rules
if service_unavailable:
    return evaluate_alternative_evidence_sources_with_partial_credit()

# Handle string errors vs dict responses
...
# Provide partial credit for service availability
elif service_accessible_but_not_configured:
    return provide_half_credit_with_guidance("⚠️ Service available but enhancement needed")

# Handle unexpected response formats gracefully
if response_format_unexpected:
    return best_effort_parsing_with_detailed_logging_and_partial_credit()
```
...
```python
# Actual graduated assessment framework from CLI assertion rules
def calculate_compliance_score(service_availability, configuration_completeness, cost_optimization_context):
    # Multiple mechanisms with different weights
    if service_availability >= 2 and configuration_completeness >= 0.8:
        return "✅ Comprehensive implementation established"
    elif service_availability >= 1.5 and configuration_completeness >= 0.6:
        return "✅ Sufficient for low-impact validation"
    elif service_availability >= 1 and configuration_completeness >= 0.4:
        return "⚠️ Basic capability (expand mechanisms recommended)"
    elif service_availability >= 0.5:  # Service available but not configured
        return "⚠️ Service available but enhancement needed"
    else:
        return "❌ Insufficient protection or capability"
```
...
```python
# Actual threshold implementation from CLI assertion rules
rule_threshold = {
    "service_available": True,          # Service can be accessed via CLI
    "basic_configuration": True,        # Fundamental security settings present
    "cost_optimization_acceptable": True, # Services may be stopped for cost management
    "partial_credit_system": True      # Half-credit for service availability, full credit for active use
}
```

**Justification Framework**:
- **Capability Demonstration**: Prove security services exist and can be properly configured and validated through CLI
- **Cost-Effective Pilot Operations**: Accommodate services configured but stopped for cost management in pilot environments
- **Graceful Service Variations**: Support diverse AWS account configurations and permission levels through robust error handling
- **NIST Low Impact Compliance**: Meet baseline requirements enhanced through systematic CLI-based validation
...
### **Specific Rule Implementation Patterns**

Based on the actual CLI assertion rules implementation, several key patterns emerge that define the validation approach:

#### **Cost-Optimization Awareness**
```python
# Pattern from actual rules: Accept services stopped for cost management
...
```python
# Pattern from actual rules: Half-credit for service availability
if service_mechanisms >= 1:
    return True, "✅ Capability established"
elif service_mechanisms >= 0.5:
    return True, "⚠️ Service available but enhancement needed"
```

#### **Multi-Mechanism Validation**
```python
# Pattern from actual rules: Multiple valid implementation paths
validation_mechanisms = [
    check_primary_service(),    # Full implementation
    check_alternative_service(), # Alternative approach
    check_basic_availability()  # Minimum capability
]
return any(mechanism.is_adequate() for mechanism in validation_mechanisms)
```

#### **Lenient Low-Impact Thresholds**
```python
# Pattern from actual rules: Explicit low-impact calibration
# Comments like "LENIENT: Accept partial configuration for low-impact"
# Comments like "For low-impact, service availability is sufficient"
# Comments like "Acceptable for pilot environments"
```

#### **Robust Error String Handling**
```python
# Pattern from actual rules: Handle AWS API error responses
if isinstance(raw_output, str):
    if "ConfigurationRecorderNotAvailable" in raw_output:
...
```python
# Pattern from actual rules: Structured manual evidence
...
```python
# Network security controls (from actual rule implementations)
network_controls = [
    "security_group_restrictiveness_validation",  # Any restrictive security groups
    "vpc_presence_and_configuration_check",       # VPC-based network segmentation
    "multi_az_deployment_assessment"              # High availability and attack surface reduction
]

# Service configuration controls (from actual rule implementations)
service_controls = [
    "ssm_parameter_store_OR_aws_config_availability",  # Centralized configuration management
    "s3_encryption_plus_ebs_encryption_validation",    # Encryption at rest
    "kms_key_availability_for_key_management"          # Automated key management
]

# Monitoring and logging controls (from actual rule implementations)
monitoring_controls = [
    "cloudtrail_configuration_regardless_of_status",   # System modification logging
    "inspector_service_availability_plus_coverage",    # Vulnerability scanning capability
    "security_hub_OR_inspector_findings_presence"      # Vulnerability tracking
]
```
...
def validate_hybrid_control(technical_evidence, administrative_evidence):
    """
    Combined technical and administrative control assessment following actual rule patterns
    """
    # Technical assessment with graceful error handling
    technical_score = 0
    if technical_evidence.service_accessible:
        if technical_evidence.properly_configured:
            technical_score = 1.0  # Full credit
        else:
            technical_score = 0.5  # Half credit for service availability
    
    # Administrative assessment with structured evidence
    administrative_score = assess_evidence_directory_structure(administrative_evidence)
    
    # Combined assessment with realistic thresholds
    if technical_score >= 0.5 and administrative_score >= 0.5:
        return ComplianceResult.PASS_WITH_MIXED_EVIDENCE
    elif technical_score >= 1.0 or administrative_score >= 1.0:
        return ComplianceResult.PASS_WITH_COMPENSATING_CONTROLS
    else:
